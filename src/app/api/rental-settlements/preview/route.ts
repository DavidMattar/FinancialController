...
/**
 * GET /api/rental-settlements/preview?from=&to=&type=DAVID|FAMILIA|LIMPEZA
 ...
 * A lógica de soma (e a divisão por 2, só no caso FAMILIA) é a mesma usada depois na
...
export async function GET(request: Request) {
  ...
  if (!from || !to || (type !== "DAVID" && type !== "FAMILIA" && type !== "LIMPEZA")) {
    return NextResponse.json(
      { error: "Informe from, to e type (DAVID, FAMILIA ou LIMPEZA)." },
      { status: 400 },
    );
  }
